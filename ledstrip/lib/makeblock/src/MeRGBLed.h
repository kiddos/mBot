...
#endif  // !ME_PORT_DEFINED
{
 public:
   ...
   * it will assigned the LED display buffer and initialization the GPIO of LED
   * lights. You can
   * set any slot for the LED data PIN, and reset the LED numberby this
   * constructor.
   * \param[in]
   *   port - arduino port
   * \param[in]
...
#endif  // ME_PORT_DEFINED
  /**
   * Destructor which can call your own function, it will release the LED
   * buffer
   */
  ~MeRGBLed(void);
   ...
   *   reset
   * \par Description
   *   Reset the LED available data PIN by its RJ25 port, and slot2 will be
   * used as default.
   * \param[in]
   *   port - RJ25 port from PORT_1 to M2
   * \par Output
   ...
   *   Set the LED color for any LED.
   * \param[in]
   *   value - the LED color defined as long type, for example (white) =
   * 0xFFFFFF
   * \par Output
   *   None
   * \return
 ...
 private:
   ...
   *   None
   */
  void rgbled_sendarray_mask(uint8_t *array, uint16_t length, uint8_t pinmask,
                             uint8_t *port);
