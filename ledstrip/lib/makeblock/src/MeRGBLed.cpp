 ...
 *    6. bool MeRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green,
 * uint8_t blue)
 *    7. bool MeRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green,
 * uint8_t blue)
...
/**
 * Alternate Constructor which can call your own function to map the MeRGBLed
 *to arduino port,
 * no pins are used or initialized here, it only assigned the LED display
 *buffer. The default
 *number of light strips is 32.
 * \param[in]
 *   None
 */
MeRGBLed::MeRGBLed(void) : MePort() { setNumber(DEFAULT_MAX_LED_NUMBER); }

/**
 * Alternate Constructor which can call your own function to map the MeRGBLed
 * to arduino port,
 * it will assigned the LED display buffer and initialization the GPIO of LED
 * lights. The slot2
 ...
 * it will assigned the LED display buffer and initialization the GPIO of LED
 * lights. You can
 * set any slot for the LED data PIN, and reset the LED number by this
 * constructor.
 * \param[in]
 *   port - RJ25 port from PORT_1 to M2
 * \param[in]
 ...
 *   led_num - The LED number
 */
MeRGBLed::MeRGBLed(uint8_t port, uint8_t slot, uint8_t led_num)
    : MePort(port) {
  if (slot == SLOT1) {
 ...
 * it will assigned the LED display buffer and initialization the GPIO of LED
 * lights. You can
 * set any arduino digital pin for the LED data PIN, The default number of
 * light strips is 32.
 * \param[in]
 *   port - arduino port
 */
 ...
 * it will assigned the LED display buffer and initialization the GPIO of LED
 * lights. You can
 * set any arduino digital pin for the LED data PIN, and reset the LED number
 * by this constructor.
 * \param[in]
 *   port - arduino port
 * \param[in]
 ...
 *   reset
 * \par Description
 *   Reset the LED available data PIN by its RJ25 port, and slot2 will be used
 * as default.
 * \param[in]
 *   port - RJ25 port from PORT_1 to M2
 * \par Output
 ...
 *   The index value from 1 to the max
 */
uint8_t MeRGBLed::getNumber(void) { return (count_led); }
 ...
 */
bool MeRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green,
                          uint8_t blue) {
  if (index < count_led) {
    uint8_t tmp = index * 3;
    pixels[tmp] = green;
 ...
 *   FALSE: Wrong execution
 * \par Others
 *   The index value from 1 to the max, if you set the index 0, all the LED
 * will be lit
 */
bool MeRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green,
                        uint8_t blue) {
  if (index == 0) {
 ...
 *   None
 */
void MeRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen,
...
void MeRGBLed::show(void) {
  rgbled_sendarray_mask(pixels, 3 * count_led, pinMask,
                        (uint8_t *)ws2812_port);
  delay(1);
}
